...
if __name__ == '__main__':
    ...
    while(cap.isOpened()):
        ...
        if(selectingObject):
            ...
            t0 = time()
            ok, boundingbox = tracker.update(frame)  #frame had better be contiguous
            t1 = time()
            ...
            if ok:
                ...
                color = (0,255,0)
            #duration = t1-t0
            text = 'FPS: '+str(1/duration)[:4].strip('.')+' OnTrack: '+str(ok)
            cv2.putText(frame, text, (8,20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
